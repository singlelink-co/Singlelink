<template>
  <div class="flex flex-col items-center justify-center bg-gray-50 min-h-screen">
    <section class="flex items-center justify-center flex-col mt-auto w-screen">
      ...
      <p class="text-gray-700 text-sm">Or, <a class="text-indigo-600 hover:text-indigo-700" href="/">check for status updates</a></p>
      <div v-if="this.error"
...
export default {
  ...
  methods: {
    attempt_register() {
      ...
      this.$axios.post('https://discord.com/api/webhooks/799417494418227270/WV22t9QsBF8gglxXYwBlsHYRA_ccY_H15QLY0GQZrsuKhwuiTIgSaVBUFsEkp1Ut2-8t',
        {
            content: 'New signup from *singlelink.co*!\n\n' +
        ...
        .then((response) => {
          return this.$router.push('/maintenance-underway/thank-you');
        })
        .catch((err) => {
