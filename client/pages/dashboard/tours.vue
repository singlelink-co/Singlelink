<template>
  <section class="flex flex-shrink-0 flex-col p-8 items-center bg-gray-100 overflow-x-hidden overflow-y-scroll">
    ...
    <p class="text-gray-600 w-full text-left mb-4">
      New to {{ app_name }}? Take our various product tours below to learn everything you need to know!
    </p>

    <div v-for="tour in tours" class="flex flex-col p-6 bg-white shadow rounded-lg w-full mb-8">
      ...
      <a
        v-if="!tour.completed"
        :href="tour.url"
        class="flex flex-row w-auto px-6 py-3 text-white font-semibold rounded-lg bg-indigo-600 hover:bg-indigo-500"
        style="width:fit-content;"
      >Begin tour</a>
      <div v-if="tour.completed" class="flex flex-row items-center justify-start">
        <a
          :href="tour.url"
          class="flex flex-row w-auto px-6 py-3 text-gray-700 font-semibold rounded-lg bg-gray-300 hover:bg-gray-200"
          style="width:fit-content;"
        >Take tour again</a>
